...
def main():
    ...
    parser.add_argument(
        '--log-file',
        dest='log_file',
        default='/tmp/VULKAN_INSTALL.log',
        help="Name of log file. Default is \'/tmp/VULKAN_INSTALL.log\'."
    )
    parser.add_argument(
        '--force-install',
        action='store_true',
    ...
    os.chdir(args.install_json_location)
    ...
    with open(log_file, 'a') as lf:
        ...
        for key in absolute_copy_files:
            ...
            if absolute_copy_files[key]["Link"] is not None:
                ...
                # I don't think this actually ever get's called
                shutil.copytree(key, absolute_copy_files[key]["Dest"], symlinks=True)
            else:
                # Remove the file first to avoid Gatekeeper problems if the old sdk
                # was not uninstalled.
                if absolute_copy_files[key]["Dir"] == False:
                    try:
                        os.remove(absolute_copy_files[key]["Dest"] + "/" + os.path.basename(key))
                    except:
                        # This is an expected (benign) failure if an SDK is not currently installed.
                        print("Could not remove " + absolute_copy_files[key]["Dest"] + "/" + os.path.basename(key))
